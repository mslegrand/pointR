library(svgR)
library(tidyverse)
WH<-c(600,400)

# Defined by mouse: edit with care!
ptR<-list(
  circle= tribble(
      ~id,         ~fill,       ~cxy,
      'yemedaxb',  '#2E0909',   matrix( c(c(299,291)), 2),
      'ncipjeju',  '#0033FF',   matrix( c(c(68,260)), 2),
      'jfnwfdhl',  '#EAFF00',   matrix( c(c(315.5,183)), 2),
      'ncipjeju',  '#AAFF00',   matrix( c(c(168.5,220)), 2),
      'ncipjeju',  '#FF00EA',   matrix( c(c(199,91)), 2),
      'tqtyjzpm',  '#0CC9C9',   matrix( c(c(485,151)), 2),
      'udrtlyzw',  '#186332',   matrix( c(c(382.5,62)), 2),
      'chmnkvnq',  '#FFAE00',   matrix( c(c(508,293)), 2),
      'rdkqglru',  'red',       matrix( c(c(67,89)), 2)
  ),
  links= tribble(
      ~fromId,     ~toId,       ~points,
      'tqtyjzpm',  'udrtlyzw',  matrix(0,2,0),
      'tqtyjzpm',  'yemedaxb',  matrix(0,2,0),
      'chmnkvnq',  'tqtyjzpm',  matrix(0,2,0),
      'cfqzzhcj',  'rdkqglru',  matrix(0,2,0),
      'rdkqglru',  'cfqzzhcj',  matrix(0,2,0),
      'japgmptb',  'ncipjeju',  matrix(0,2,0),
      'japgmptb',  'jfnwfdhl',  matrix(0,2,0)
  )
)

arrs<-ptR$links %>% 
  inner_join( ptR$circle, by=c('fromId'='id') )%>%
  inner_join( ptR$circle, by=c('toId'='id') ) %>%
  select(c(5,7)) %>% 
  rename(p1=cxy.x, p2=cxy.y) 
 
 
#print(arrs) 

# print(arrs$cxy.x)
# print(arrs$cxy.y)

 R=50
 

arrow%<c-%function(p1, p2, R=50){
  v=p2-p1
  L=sqrt(sum(v^2))
  xy1=p1 + v*(R/L)
  xy2=p2 - v*((10+R)/L)
  line(xy1=xy1, xy2=xy2, 
    stroke='black', stroke.width=2, 
    marker.end=marker(viewBox=c(0, 0, 10, 10), refXY=c(1,5), 
      markerWidth=6, markerHeight=6, orient="auto",
      path( d=c("M", 0, 0, "L", 10, 5, "L", 0, 10, "z") )
    ) 
  )
}


svgR(wh=WH,
    #your custom code goes here
    pmap(ptR$circle, circle, r=R, opacity=.5),
    pmap(arrs, arrow )
    #length(tmp)
#     arrow(ptR$circle$cxy[[1]], ptR$circle$cxy[[2]], R)
)

